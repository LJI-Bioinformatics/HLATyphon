...
my $overall_detail_file  = $OUTPUT_Dir."/Overall_Detail_Bases.txt";
...
print OVERALL "Allele_Combinations_Mapped_Bases\n";
...
while (my ($Loci, $Loci_Samples) = each %HLA_type) {
	...
	LOCI: foreach my $sample_ID (sort {$a cmp $b} @{$Loci_Samples}) {
		...
		system("mkdir -p $parent_Dir") if (! -d $parent_Dir);
		
		my $countFile          = $parent_Dir."/count_2.txt";
		my $truncatedCountFile = $parent_Dir."/HLA_type.txt";
		
		if (! -e $countFile) { $countFile = $truncatedCountFile; }
		
		my $sortedCountFile    = $parent_Dir."/Sorted_Count_Bases.txt";
		my $collapseCountFile  = $parent_Dir."/Collapse_Count_Bases.txt";
