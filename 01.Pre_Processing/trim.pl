#!/home/zfu/apps/Perl_5_16_3/bin/perl
...
--offset <num>	in case of type 2 trimming, trim the beginning bases, default 5
...
while ($p1_header1 = <PAIR1>) {
	...
	if ($type == 0) {
		...
		$paired_cnt++;
                ...
                $p1_seq=substr($p1_seq,$offset,length($p1_seq));
                $p2_seq=substr($p2_seq,$offset,length($p2_seq));
                $p1_qual=substr($p1_qual,$offset,length($p1_qual));
                $p2_qual=substr($p2_qual,$offset,length($p2_qual));
		...
		foreach $qual_char (split (//, $p1_qual)) {
			...
			$pos++;
                ...
                if(length($p1_seq) < $length_threshold){
                    $p1_flag = 0;
                }
                ...
                if(length($p2_seq) < $length_threshold){
                    $p2_flag = 0;
                }
